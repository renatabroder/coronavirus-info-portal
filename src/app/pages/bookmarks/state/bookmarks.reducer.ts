...
const reducer = createReducer(
  ...
  on(fromBookmarksAction.removeBookmark, (state, { stringBusca }) => ({
    ...state,
    list: state.list.filter(b => b.stringBusca !== stringBusca),
  })),
);
...
function toggleBookmark(list: Bookmark[], entity: Bookmark): Bookmark[] {
  if (!!list.find(bookmark => bookmark.stringBusca === entity.stringBusca)) {
    return list.filter(bookmark => bookmark.stringBusca !== entity.stringBusca);
  }
  return [...list, entity];
}
